...
function createTeamBlock(teamNumber) {
  ...
  const teamData = teamDataMap.get(teamNumber);
  const isLocked = teamData && teamData.players && teamData.players.length > 0;
  const playerNames = isLocked ? teamData.players : [];
